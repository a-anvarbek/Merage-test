  ...
  return (
    <section className="relative bg-nippon-black section-padding overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div
          className="absolute top-1/3 left-1/4 w-96 h-96 rounded-full bg-nippon-gold opacity-2 blur-3xl"
          data-scroll-parallax
          data-scroll-speed="0.1"
        ></div>
        <div
          className="absolute bottom-1/4 right-1/3 w-64 h-64 rounded-full bg-nippon-gold opacity-3 blur-3xl"
          data-scroll-parallax
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="space-y-12" data-scroll-reveal>
            ...
            <div className="grid grid-cols-3 gap-8 pt-8">
              {[
                { symbol: '心', title: 'Heart', subtitle: 'Kokoro' },
                { symbol: '和', title: 'Harmony', subtitle: 'Wa' },
                { symbol: '礼', title: 'Respect', subtitle: 'Rei' }
              ].map((item, index) => (
                <div key={index} className="text-center">
                  <div className="w-16 h-16 border-2 border-nippon-gold mx-auto mb-4 flex items-center justify-center gold-glow">
                    <span className="text-nippon-gold font-serif text-luxury-2xl">{item.symbol}</span>
                  </div>
                  <p className="text-nippon-warm-white font-sans text-luxury-sm">{item.title}</p>
                  <p className="text-nippon-gray font-sans text-luxury-xs">{item.subtitle}</p>
                </div>
              ))}
            </div>
          </div>
