...
export function Navigation({ currentSection, onSectionChange }) {
  ...
  const navigationItems = [
    ...
    { id: 'experiences', label: 'Experiences', submenu: [
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-nippon-black/95 backdrop-blur-lg shadow-luxury border-b border-nippon-gold/20' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between h-20">
            <div className="cursor-pointer" onClick={() => onSectionChange('home')}>
              <h1 className="text-luxury-2xl text-nippon-gold font-serif tracking-wider">Nippon Imperial</h1>
            </div>
              ...
              {navigationItems.map(item => (
                <div key={item.id} className="relative group">
                  <button
                    ...
                    className={`text-luxury-sm font-sans tracking-wide transition-all duration-300 hover:text-nippon-gold ${currentSection === item.id ? 'text-nippon-gold' : 'text-nippon-warm-white'}`}
                  >
                    {item.label}
                  </button>
                  {item.submenu && (
            ...
            <Button className="lg:hidden p-2 bg-transparent text-nippon-gold hover:bg-nippon-gold/10" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            ...
            <div className="max-w-7xl mx-auto px-6 py-6 space-y-4">
              {navigationItems.map(item => (
                <div key={item.id}>
                  <button
                    ...
                    className={`block w-full text-left py-3 text-luxury-base font-sans transition-colors duration-200 ${currentSection === item.id ? 'text-nippon-gold' : 'text-nippon-warm-white hover:text-nippon-gold'}`}
                  >
                    {item.label}
                  </button>
                  {item.submenu && (
                    <div className="ml-4 space-y-2">
                      {item.submenu.map(subItem => (
                        <button
                          key={subItem.id}
                          onClick={() => { onSectionChange(subItem.id); setIsMenuOpen(false); }}
              ...
              <div className="pt-4 border-t border-nippon-gold/20">
                <div className="space-y-3 text-luxury-sm text-nippon-gray-light">
                  <div className="flex items-center space-x-3"><Phone className="w-4 h-4 text-nippon-gold" /><span>+81 3-1234-5678</span></div>
                  <div className="flex items-center space-x-3"><span>concierge@nipponimperial.com</span></div>
                </div>
              </div>
            </div>
          </div>
